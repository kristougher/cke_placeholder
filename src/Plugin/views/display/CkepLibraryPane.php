...
namespace Drupal\cke_placeholder\Plugin\views\display;

use Drupal\Core\Form\FormStateInterface;
use Drupal\views\Plugin\views\display\DisplayPluginBase;

/**
 * The plugin that handles CKE Placeholder media library display.
 *
 * "ckep_library_pane" is a custom property, used with
 * \Drupal\views\Views::getApplicableViews() to retrieve all views with
 * this display.
 *
 * @ingroup views_display_plugins
 *
 * @ViewsDisplay(
 *   id = "ckep_library_pane",
 *   title = @Translation("CKE Placeholder library"),
 *   help = @Translation("Displays a view in the CKE Placeholder library."),
 *   theme = "views_view",
 *   admin = @Translation("CKE Placeholder Library View"),
 *   ckep_library_pane = TRUE
 * )
 */
class CkepLibraryPane extends DisplayPluginBase {
  ...
  public function elementPreRender(array $element) {
    ...
    if (!is_array($element['#rows'])) {
      ...
      foreach ($element['#rows'][0]['#rows'] as &$row) {
        $row = [
          '#theme' => 'cke_placeholder_library_view_item',
          '#preview' => $row,
          '#cke_placeholder_tag' => 'drupal_entity',
          '#data' => [
            'type' => 'entity',
            'uuid' => '1233456',
          ],
    ...
    drupal_set_message(print_r(array_keys($element['#rows'][0]['#rows'][0]), 1), 'status', FALSE);
